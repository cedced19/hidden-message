...
gulp.task('js-guess', function () {
    var b = browserify({
      entries: 'dev/scripts/guess.js',
      debug: true
    });
  
    return b.bundle()
      .pipe(source('dev/scripts/guess.js'))
      .pipe(buffer())
      .pipe(isDist ? uglify() : through())
        .on('error', log.error)
      .pipe(rename('guess.min.js'))
      .pipe(gulp.dest('scripts/'));
});
...
gulp.task('js', ['js-guess', 'js-index']);
...
gulp.task('serve', function () {
    ...
    gulp.watch('dev/styles/main.css', ['css-watch']);
    gulp.watch('dev/scripts/index.js', ['js-index-watch']);
    gulp.watch('dev/scripts/guess.js', ['js-guess-watch']);
});
